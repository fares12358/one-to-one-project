...
// Self-service email/password change for the logged-in admin. Requires the
// CURRENT password as a re-auth step — this is not a plain profile update.
export const updateCredentials = async (req, res, next) => {
  try {
    ...
    // req.admin (from the middleware) has password stripped — re-fetch to compare
    ...
    if (newPassword) admin.password = newPassword; // re-hashed by the model's pre("save") hook

    await admin.save();
    clearAdminCache(admin._id);

    // Reissue the session cookie so its embedded `email` claim doesn't go
    // stale mid-session (same shape as login's token).
    const token = jwt.sign(
      { id: admin._id, email: admin.email },
      process.env.JWT_SECRET,
      { expiresIn: process.env.JWT_EXPIRES_IN || "7d" }
    ...
    if (newPassword) {
      sendPasswordResetConfirmation(admin.email).catch((err) =>
        console.warn("[auth] Credential-change confirmation email failed:", err.message)
      );
    }
