...
// Builds a transporter (+ from/to) from the DB-stored emailConfig when an
// admin has configured one via the dashboard, falling back to the legacy
// EMAIL_* env vars otherwise — zero-downtime for deployments that haven't
// migrated yet. Built fresh per call: this app's email volume (contact form
// + password reset) doesn't justify a caching/invalidation layer, and it
// means a dashboard save takes effect on the very next email sent.
const envTransporter = () =>
...
const resolveMailer = async () => {
  ...
  if (cfg?.passEncrypted) {
    try {
      transporter = nodemailer.createTransport({
        ...
        port:   cfg.port,
        secure: cfg.secure,
        auth: { user: cfg.user, pass: decrypt(cfg.passEncrypted) },
      });
    } catch (err) {
      // Most likely JWT_SECRET rotated since this password was encrypted —
      // fall back to env vars instead of failing every outbound email.
      console.warn("[email] Failed to decrypt stored emailConfig password, falling back to env vars:", err.message);
  ...
  return {
    transporter,
    from: cfg?.from || process.env.EMAIL_FROM,
    to:   cfg?.to   || process.env.EMAIL_TO,
  };
};
...
export const sendContactEmail = async (submission) => {
  ...
  const { transporter, from, to } = await resolveMailer();
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;border:1px solid #e5e7eb;border-radius:12px">
        <div style="background:#037338;padding:16px 24px;border-radius:8px;margin-bottom:24px">
          ...
          <p style="color:#96C422;margin:4px 0 0;font-size:14px">One to One Website</p>
        </div>
...
export const sendPasswordResetEmail = async (adminEmail, resetUrl) => {
  const { transporter, from } = await resolveMailer();

  await transporter.sendMail({
    from,
    to:      adminEmail,
    subject: "Reset your One to One admin password",
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;border:1px solid #e5e7eb;border-radius:12px">
        <div style="background:#037338;padding:16px 24px;border-radius:8px;margin-bottom:24px">
          <h1 style="color:#fff;margin:0;font-size:20px">Password Reset Request</h1>
          <p style="color:#96C422;margin:4px 0 0;font-size:14px">One to One Admin</p>
        </div>
        <p style="color:#374151">You requested a password reset for your One to One admin account.</p>
...
export const sendPasswordResetConfirmation = async (adminEmail) => {
  ...
  await transporter.sendMail({
    from,
    to:      adminEmail,
    subject: "Your One to One admin password has been changed",
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;border:1px solid #e5e7eb;border-radius:12px">
        <div style="background:#037338;padding:16px 24px;border-radius:8px;margin-bottom:24px">
        ...
        <p style="color:#374151">Your One to One admin password was successfully changed at:</p>
